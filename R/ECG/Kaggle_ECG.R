...
# Create the graph
scaling_factor <- 0.2 # Scale the gaps by this factor to transfer the graph from one dataset to another
qrs_graph <- gfpop::graph(
  gfpop::Edge(0, 1, type = "down", penalty = 8e7, gap=0),
  gfpop::Edge(1, 2, type = "up", penalty = 0, gap=2*scaling_factor),
  gfpop::Edge(2, 3, type = "down", penalty = 0, gap=3*scaling_factor),
